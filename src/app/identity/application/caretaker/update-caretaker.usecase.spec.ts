...
import { UpdateCaretakerUseCase } from './update-caretaker.usecase';

/**
 * Edição de responsável (T087): pelo identificador, sem senha.
 */
describe('UpdateCaretakerUseCase', () => {
  ...
  function refusal<T>(): Result<T> {
    return failure<T>(
      Notification.of([
        { code: 'LAST_ADMINISTRATOR', field: 'role', message: 'O sistema precisa de ao menos um administrador ativo.' },
      ]),
    );
  }
  ...
  const update = {
    ...
    cpf: '52998224725',
    email: 'joao@ovyx.com.br',
    mobilePhone: '91991234567',
    role: 'ADMINISTRATOR' as const,
  };

  it('updates the caretaker by its id, with what was typed', async () => {
  ...
  it('passes the backend refusal through, without rewriting it', async () => {
    ...
    gateway.update.mockResolvedValue(refusal());

    const result = await TestBed.inject(UpdateCaretakerUseCase).execute(joao.id, update);
